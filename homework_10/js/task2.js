...
function fighter(stats){
  ...
  Fighter.getName = function(){
    return info.name;
  }
  Fighter.Block = function(incomingAttack){
    return Math.random() > 0.5;
  }
  Fighter.getStats = function(){
    return info;
  }
  Fighter.getCombatHistory = function(){
    return combatHistory;
  }
  Fighter.fight = function(defender){
    if (defender.Block()){
      return false;
    } else {
      if (info.attack >= defender.getStats().hp){
        defender.getStats().hp = 0;
        combatHistory.wins++;
        defender.getCombatHistory().loses++;
      }else{
        defender.getStats().hp -= info.attack; 
      }
      return true;
    }
  }
